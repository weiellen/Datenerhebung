...
function connect485port(){
		 ...
		 .then(retrieveAvailableSensorIDs, error=>{logger.log('error', 'RaspberryPi doesnot find RS485 serial port.\n'); });
}

/*Retreive IDs of all available devices*/
function retrieveAvailableSensorIDs(){
	      modbusSensorClient.setID(deviceId); 
	      modbusSensorClient.setTimeout(1000);
	      modbusSensorClient.readInputRegisters(0, 3, (err, data) =>{
		      if(err){
					  logger.log('info', 'no device '+deviceId); 
					  if(deviceId<check_device_id_end){
					     deviceId++;
					     retrieveAvailableSensorIDs();
				      }
				      else{
						  //if no devices found, repeat to search from the begin every 5 minutes.
						  if(availableDevice.length==0){
							 logger.log('info', 'no devices are found. try it after 5 minautes'); 
				             deviceId=check_device_id_start;
					         setInterval(retrieveAvailableSensorIDs, 300000);
					        }
					        else{
                                 executePolling();
							}
...
function executePolling(){
						  logger.log('info', availableDevice.length+' devices are found!');
					       modbusSensorClient.setTimeout(2000); 
		                   ...
		                   ftp.on('error', function(){
                                     ...
                                     setInterval(polling_read_register, 60000);    
			                        }    
	                            );
...
function  polling_read_register(){ 
		        ...
		        modbusSensorClient.setID(availableDevice[counter]);
		       //write file	
