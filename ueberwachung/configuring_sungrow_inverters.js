...
1. polling all available sungrow inverters to retrieve their device ids;
2. Configuring Power limitation at adresses 5007-1 = 5006 and 5008-1=5007 for each available inverter;
3. Confirming the setting through console output;
*************************************************************************************************************************************************************************/
...
/*create logger*/
logger = new Logger ("debug", "debug",  "log/configuring_sungrow_inverters")
...
var check_device_id_end = 50; // actually possisble maximal slave address is 247
/*initialize the start id as 1*/
var deviceId = check_device_id_start;
/*Index of array of available inverters*/
var counter = 0;
...
function start(){
          ...
          exec('cat /proc/cpuinfo | grep Serial', (error, stdout, stderr) => {
	                ...
	                .then(retrieveAvailableInverterIDs, error=>{logger.log('error', 'RaspberryPi doesnot find RS485 serial port.');  });
         });
}

/*STEP 2 Retreiving IDs of all available inverters recursively*/
function retrieveAvailableInverterIDs(){
	      ...
	      var status = modbusInverterClient.readInputRegisters(5037, 1, (err, data) =>{
		      if(err){
				      ...
				      else{//if no devices found, repeat to search from the begin every 5 minutes	  
						  if(availableDevice.length==0){
							  logger.log('info', 'no devices are found. try it after '+parseInt(connectingFrequence/60000, 10)+' minutes.\n'); 
				             deviceId=check_device_id_start;
					         retrieveIdsInvervalObject = setInterval(retrieveAvailableInverterIDs, connectingFrequence);
					        }
					        else{
				                 logger.log('info', availableDevice.length+' devices found!');
...
async function  enableSetting(){ 
		   logger.log('info', 'try to open the '+(counter+1)+'th device with ID '+availableDevice[counter]);
		   await modbusInverterClient.setID(availableDevice[counter]);
		    var actualDatum = new Date().toLocaleDateString().trim();
		    
	        //IMPORTANT: for writing register, sungrow supports the command code 0x10 i.e. WRITE REGISTERS   
	       await modbusInverterClient.writeRegisters(5006, [170]) // 0xAA enable
           .then(settingPowerLimit, error=>{logger.log('error', 'Cannot enable the power limit of the inverter with ID '+availableDevice[counter]+'\n');});        
           
           //then set the next inverter 									    
...
function settingPowerLimit() {
           modbusInverterClient.writeRegisters(5007, [parseInt(powerLimit)])  
          .then(console.log);
}
